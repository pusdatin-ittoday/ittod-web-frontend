...
- The landing competition timeline uses the 2026 competition schedule phases: first registration, second registration, finalist announcement, and final & awarding.
...
- The API-backed competition detail route mirrors the event-detail composition, including the Admin/API logo hero, responsive information sidebar, and compact contact card. The previous decorative `URGENT` badge has been removed without changing competition fetching, guidebook, or registration behavior.
- Event and competition detail pages replace their former static date/deadline and generic information cards with one shared Neo-Brutalist agenda panel. The panel consumes the ordered `timelines` array already returned by the event-detail API, formats each schedule in the Asia/Jakarta timezone, and renders a safe empty state until the responsible Admin or competition committee adds agenda records.
...
- The public navbar reads `AuthContext`, which now hydrates immediately from shared login hints (`sessionStorage.userData`, `authToken`, and the legacy `isLoggedIn/userId` flags) before refreshing current-user data through the backend authentication-status endpoint. This keeps authenticated users on the green `PROFILE` action when navigating from dashboard back to public Home/Event/Competition/Contact Us pages, while logout clears every stored auth hint before redirecting to `/login`.
...
- Dashboard member-level rejection notes keep the desktop hover tooltip, but mobile viewports render the member's `verificationError` directly below the `REJECTED` label because touch devices do not have reliable hover access.
...
2. **Dynamic Event Data**:
   ...
   - Competition detail pages apply the same rule: unauthenticated visitors are sent to Login first, individual registration then asks for confirmation and submits directly, while team registration opens the team-name form. The public detail CTA normalizes Admin/API participation-type values such as `individual`, `individu`, and alternate camelCase field names so individual competitions do not fall back to the team flow.
   ...
   - Team creation supports both `/dashboard/lomba/:competitionId/register` and the legacy `/register-competition/:competitionSlug` path. Public competition detail CTAs now point to the protected dashboard registration route, while the form accepts either route parameter and resolves the competition through the existing public events API.
   ...
   - Edit Profile sets explicit mobile keyboard hints per field: name, institution, and social IDs use text keyboards, phone uses the telephone keyboard, and birth date keeps the native date picker.
