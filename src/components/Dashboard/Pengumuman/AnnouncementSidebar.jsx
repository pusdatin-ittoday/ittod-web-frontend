        ...
        <aside className="w-full xl:w-[340px] flex-shrink-0 flex flex-col gap-5">
            {isUrgent ? (
                <div className="border-[4px] border-[#1A1C1C] bg-[#FFDAD6] p-5 shadow-[6px_6px_0_#000] flex flex-col gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-inter font-black text-base uppercase text-[#93000A]">URGENT ALERT</span>
                    </div>
                    <div>
                        <p className="font-inter font-black text-xl text-[#93000A] uppercase leading-tight">
                            {closestDeadline.timelineTitle}
                        </p>
                        <p className="font-inter font-black text-xl text-[#93000A] uppercase">
                            DALAM {closestDeadline.daysUntil} HARI!
                        </p>
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <p className="font-inter font-medium text-[13px] text-[#93000A]">
                ...
                <div className="border-[4px] border-[#1A1C1C] bg-[#F0F0FF] p-5 shadow-[6px_6px_0_#000] flex flex-col gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-inter font-black text-base uppercase text-[#34399F]">UPCOMING</span>
                    </div>
                    <p className="font-inter font-black text-lg text-[#34399F] uppercase leading-tight">
                        {closestDeadline.timelineTitle}
                    </p>
                    <p className="font-inter font-medium text-[13px] text-[#464652]">
            ...
            {urgentItems.length > 1 && (
                <div className="border-[4px] border-[#1A1C1C] bg-[#F4F4F2] p-5 shadow-[6px_6px_0_#000] flex flex-col gap-4">
                    <p className="font-inter font-bold text-sm uppercase text-[#1A1C1C] tracking-wide">
                        Semua Deadline
                    </p>
                    <div className="flex flex-col gap-3">
                        {urgentItems.slice(0, 4).map((item, i) => (
                            <div key={i} className={`flex flex-col gap-0.5 pb-3 ${i < Math.min(urgentItems.length, 4) - 1 ? 'border-b border-[#1A1C1C]' : ''}`}>
                                <p className="font-inter font-bold text-xs text-[#34399F] uppercase">
                                    {formatDate(item.date)}
                                </p>
                                <p className="font-inter font-medium text-sm text-[#1A1C1C]">
