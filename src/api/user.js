...
export const getUserData = async () => {
  const userData = await instance.get('/api/user/data', { withCredentials: true })
...
/**
 * Upload image to R2 storage (key is generated by backend)
 * @param {File} file - The image file to upload
 * @returns {Promise<Object>} Result of the upload, including the generated key
 */
export const uploadImageToR2 = async (file) => {
  try {
    ...
    return {
      success: false,
      error: error.response?.data?.message || "Failed to upload image to R2"
    };
  }
};
