...
import { submitCompetitionFile } from "../../api/compeFile";
...
const Submit_Uxtoday = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Submit each link separately to the API
      const uxSubmission = {
        title: "Submisi UXToday",
        url_file: {
          TrailerKarya,
          Proposal,
        },
      ...
      // Submit the file
      const trailerResult = await submitCompetitionFile(uxSubmission);
      if (!trailerResult.success) {
        throw new Error(`Failed to submit trailer: ${trailerResult.error}`);
      }
    ...
    const fetchTeamId = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          const uxTodayTeam = result.data.find(
            (comp) => comp.competitionName?.toLowerCase() === "uxtoday"
          );
        ...
        <div className="bg-[#7b446c] text-white rounded-2xl shadow-lg p-8 max-w-3xl w-full">
          ...
          >
            <div className="mb-3 relative">
              <label
                htmlFor="TrailerKarya"
                className="block text-sm font-bold mb-2"
              >
                Trailer Karya (youtube)
              </label>
