		...
		const fetchTeamIdAndSubmission = async () => {
			try {
				...
				if (result.success && result.data) {
					...
					const gameTodayTeam = result.data.find(
						(comp) => comp.competitionName?.toLowerCase() === "gametoday"
					);
					...
					if (gameTodayTeam && gameTodayTeam.teamID) {
						...
						if (gameTodayTeam.submissionData) {
							const submissionData =
								gameTodayTeam.submissionData.submission_object || {};

							// Set form fields with existing data
							if (submissionData.SubmisiGame)
								setSubmisiGame(submissionData.SubmisiGame);
						}
					} else {
